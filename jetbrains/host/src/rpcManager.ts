	...
	public setupDefaultProtocols(): void {
		...
		this.rpcProtocol.set(MainContext.MainThreadFileSystem, {
			...
			async $readFile(resource: UriComponents): Promise<any> {
				...
				try {
					// Note: draft:// URIs are handled by the plugin's MainThreadFileSystem
					// which is invoked via the RPC protocol. This stub only handles file:// URIs.
					const filePath = this.uriToPath(resource)
		...
		this.rpcProtocol.set(MainContext.MainThreadLanguageModelTools, {
			...
			$createDraft(name: string, content: string): Promise<any> {
				console.log("Creating draft via RPC:", { name, content })
				// This method is called by the TypeScript CreateDraftTool when running in JetBrains mode.
				// The draft creation is handled by the Kotlin plugin's MainThreadLanguageModelTools.createDraft().
				// We return a mock success response - the actual draftPath is generated by the Kotlin side.
				// Generate a draft path that the TypeScript side can use
				const filename = name.endsWith(".md") ? name : `${name}.md`
				const draftPath = `draft://${filename}`
				return Promise.resolve({ success: true, draftPath })
			},
			dispose(): void {
